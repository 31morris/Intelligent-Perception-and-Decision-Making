...
class STP3(nn.Module):
    ...
    def warp_features(x, affine_mats):
        ...
        # 檢查輸入形狀
        N, C, H, W = x.shape
        assert affine_mats.shape == (N, 2, 3), "affine_mats 應該具有形狀 (N, 2, 3)"

        # 創建仿射網格（Affine Grid）
        # 注意：affine_grid 接受的矩陣為 (N, 2, 3)，輸出為 (N, H, W, 2)
        grid = F.affine_grid(affine_mats, size=(N, C, H, W), align_corners=False)
        
        # 應用仿射網格進行特徵圖采樣
        # grid_sample 根據生成的 grid 對輸入 x 進行變換
